...
build-images: ## Build the images
	docker buildx build --platform linux/arm64 --tag "ghcr.io/k8sdeploy/tygon:`git rev-parse HEAD`" --build-arg build=`git rev-parse HEAD` --build-arg version=`git describe --tags --dirty` --file ./k8s/Dockerfile .
	docker tag "ghcr.io/k8sdeploy/tygon:`git rev-parse HEAD`" "ghcr.io/k8sdeploy/tygon:latest"
	docker scan "ghcr.io/k8sdeploy/tygon:`git rev-parse HEAD`"
